...
export interface UpgradeOption {
  ...
  price: number;
  originalPrice?: number; // New: Store original price for inflation display
  icon: string;
...
export interface EnemyConfig {
  ...
  burstPhrases?: string[]; // New: For burst attacks
...
export interface CharacterConfig {
  ...
  storyScenes: { type: 'left' | 'right' | 'center', emoji: string, name: string, text: string }[];
}
...
export interface Player extends Entity {
  ...
  isRegeneratingShield: boolean; // New: For UI feedback
  invulnerableTime: number;
  items: string[]; 
  
  insuranceGoldEarned: number; // New: Track gold earned from insurance

  isDying: boolean; // New: Death animation state
...
export interface Enemy extends Entity {
  ...
  aimX?: number;
  aimY?: number;
  stunTimer?: number; // New: For stun effect
  burstQueue?: string[]; // New: Queue for burst characters
  burstTimer?: number; // New: Timer between burst shots
}
