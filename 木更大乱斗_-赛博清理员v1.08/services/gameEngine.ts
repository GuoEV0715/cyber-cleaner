...
import { ENEMIES, BULLET_TEXTS, WAVES, CHARACTERS, DIFFICULTY_SETTINGS } from "../data/memeContent";

// Simple Audio Synthesizer & BGM
export class SoundManager {
  ctx: AudioContext | null = null;
  nodes: AudioNode[] = [];
  currentBgmType: 'battle' | 'shop' | 'story' | 'boss' | 'ambient' | 'none' = 'none';
  ...
  playAmbientBGM() {
      if (!this.ctx || this.currentBgmType === 'ambient') return;
      this.stopBGM();
      this.currentBgmType = 'ambient';

      // Minecraft C418 Style: Ambient, slow, sparse, C Major / A Minor
      const scale = [261.63, 329.63, 392.00, 493.88, 523.25]; // CMaj7
      
      const playNote = () => {
          if (this.currentBgmType !== 'ambient' || !this.ctx) return;
          
          const osc = this.ctx.createOscillator();
          const gain = this.ctx.createGain();
          
          // Sine for pure, soft tone
          osc.type = 'sine';
          
          // Random note from scale
          let freq = scale[Math.floor(Math.random() * scale.length)];
          if (Math.random() > 0.5) freq /= 2; // Lower octave often
          ...
          gain.gain.linearRampToValueAtTime(0.05, now + 2.0); // 2s attack
          gain.gain.exponentialRampToValueAtTime(0.001, now + 8.0); // 6s decay
  ...
  playStoryBGM() {
      ...
      const playNote = () => {
          if (this.currentBgmType !== 'story' || !this.ctx) return;
          const osc = this.ctx.createOscillator();
          const gain = this.ctx.createGain();
          osc.type = 'triangle';
...
export class GamePhysics {
  ...
  startWave(waveNum: number) {
    ...
    // Inflation: 5% increase per wave (linear)
    this.state.inflationRate = (this.state.currentWave - 1) * 0.05;
  ...
  private updateWaveLogic() {
      ...
      if (waveConfig.isBossWave && !this.state.isEndless) {
          ...
          if (boss && !boss.isTransitioning && this.state.waveTimer % 300 === 0) {
               this.spawnEnemyRandomly('minion');
               if (boss.phase === 2) {
                   this.spawnEnemyRandomly('minion'); 
               }
          }
  ...
  private triggerWaveEnd() {
      ...
      const effectiveWaveNum = this.state.isEndless ? ((this.state.endlessWaveCount - 1) % 7) + 1 : this.state.currentWave;
      if (!WAVES.find(w => w.waveNumber === effectiveWaveNum)?.isBossWave) {
          this.state.enemies = [];
      }
      this.state.projectiles = [];
  }
  ...
  private updatePlayer() {
    ...
    // Item Effects: Involution King Bleed
    if (p.items.includes('卷王')) {
        if (this.state.timeAlive % 60 === 0 && p.hp > 1) {
            p.hp -= 1;
            this.spawnFloatingText(p.x, p.y - 20, "-1", "#ef4444");
        }
    }
  ...
  private shoot() {
    ...
    const createBullet = (dir: number) => {
        const text = bulletEmoji || BULLET_TEXTS[Math.floor(Math.random() * BULLET_TEXTS.length)];
         ...
         if (proj.isEnemy) {
             ...
             for (const e of this.state.enemies) {
                 ...
                 if (dist < proj.radius + e.radius) {
                     ...
                     if (p.items.includes('意外险')) {
                         p.gold += 5;
                         this.spawnFloatingText(e.x, e.y - 20, "💰+5", "#fbbf24", 'gold');
                     }
